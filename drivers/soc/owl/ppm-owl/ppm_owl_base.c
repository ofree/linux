...
注：
1.xcore @ highest value we can only keep serveral minutes

*/
/*
    mCurPolicyParam.cpu_core = 4;
    mCurPolicyParam.cpuLevel = FREQ_LEVEL_5;
    mCurPolicyParam.gpuLevel = 2;
    mCurPolicyParam.startMode = START_MODE_NORMAL;	//
    mCurPolicyParam.boost    = 0;	//performance governor
    mCurPolicyParam.timeOpt    = 0;

*/
/*
	cpufreq setting:
	1.scaling_max_freq & scaling_min_freq
	2.loading calculate, io_is_busy
...
int set_cpu_performance_level(int level, int governor, int core)
{
	//get governor & cpu range by level
	//set governor & cpu range
}

int restore_cpu_performance_level(void)
{
}
*/
...
int set_gpufreq_range(int min, int max)
{

}

int restore_gpufreq_range(int min, int max)
{

}

int set_gpu_performance_level(int level, int governor)
{
	//get range by level
	//set gpu range
}

int restore_gpu_performance_level(void)
{
	//get range by level
	//set gpu range
}
...
/* #define GOVERNOR_FIXED_MAX_FREQ "performance"
#define GOVERNOR_AUTO_FREQ	"interactive"

int set_governor(char *governor_str)
{
	int ret=0;
	unsigned char governor[12];
	struct cpufreq_policy *policy;

	strcpy(governor, governor_str);
	policy = cpufreq_cpu_get(0);
	if(policy != NULL)
	{
		store_scaling_governor(policy, governor, 12);
		pr_info("%s\n", __func__);
...
{
	pr_info("%s\n", __func__);
	return set_governor(GOVERNOR_FIXED_MAX_FREQ);
}
