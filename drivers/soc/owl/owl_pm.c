...
#define OWL_PM_REG_BASE			0xe00f8000	/* CPU_CTL_BASE */
#define OWL_PM_WAKEUP_SRC_REG		(0x017c)	/* LWER_EL_AARCH64_IRQ_VECTOR */
#define OWL_PM_FLAG_REG			(0x0180)	/* LWER_EL_AARCH32_IRQ_VECTOR */
...
static void owl_pm_set_reboot_tgt(unsigned int tgt)
{
	...
	owl_pm_clear_flag(OWL_PM_FLAG_REBOOT_TGT((1 << OWL_PM_FLAG_REBOOT_TGT_WIDTH) - 1));
...
static int __init owl_pm_init(void)
{
	...
	pm_reg_base = ioremap(OWL_PM_REG_BASE, 0x1000);
