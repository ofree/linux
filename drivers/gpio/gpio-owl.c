...
#define GPIO_CTLR_PENDING_MASK		(0x11111111)
...
/* lock for shared register between banks,  e.g. share_intc_ctlr in s500 */
...
static inline int is_s500_gpio(struct owl_gpio_bank *bank)
{
	return bank->devid == S500_GPIO;
}
...
static unsigned int read_intc_ctlr(struct owl_gpio_bank *bank)
{
	...
	/* s500: all banks share one INTC_CTLR register */
	if (is_s500_gpio(bank)) {
		val = readl_relaxed(bank->base + bank->regs->intc_ctlr);
		val = (val >> (4 * bank->id)) & 0xf;
	} else
		val = readl_relaxed(bank->base + bank->regs->intc_ctlr);
...
static void owl_gpio_irq_handler(struct irq_desc *desc)
{
	...
	for_each_set_bit(i, &pending, 32) {
		...
		/* clear pending */
		writel(BIT(i), bank->base + bank->regs->intc_pd);
	}
...
static const struct owl_gpio_bank_data s500_banks_data[5] = {
...
static const struct of_device_id owl_gpio_dt_ids[] = {
	...
	{ .compatible = "actions,s500-gpio", .data = (void *)S500_GPIO, },
	{ },
};
...
static int owl_gpio_probe(struct platform_device *pdev)
{
	...
	} else if (is_s500_gpio(bank)) {
		BUG_ON(bank->id >= ARRAY_SIZE(s500_banks_data));
		pdata = &s500_banks_data[bank->id];
	} else {
		return -ENOENT;
	}
