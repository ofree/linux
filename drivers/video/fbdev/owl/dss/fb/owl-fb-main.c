...
static void __ofbi_set_video_info(struct owl_fb_info *ofbi)
{
	...
	v_info.global_alpha = 0xff;
	v_info.global_alpha_en = true;
	v_info.pre_mult_alpha_en = false;
...
static int owl_fb_set_par(struct fb_info *fbi)
{
	...
	/* apply path info and video info, should be renamed */
...
static int owl_fb_init_ofbi(struct owl_fb_info *ofbi)
{
	...
	video = owl_de_video_get_by_id(ofbi->id);
