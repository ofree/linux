/*
 * Actions OWL SoCs SIRQ interrupt controller driver
 *
...
static struct owl_sirq_info s500_sirq_info[OWL_MAX_NR_SIRQS] = {
	{ .reg = S500_INTC_EXTCTL + 0, .share_reg = true },
	{ .reg = S500_INTC_EXTCTL + 1, .share_reg = true },
...
static unsigned int sirq_read_extctl(struct owl_sirq_info *sirq)
{
	...
	if (sirq->share_reg)
		val = readb_relaxed(sirq->base + sirq->reg);
	else
...
{
	if (sirq->share_reg)
		writeb_relaxed(extctl, sirq->base + sirq->reg);
	else
...
int __init s500_sirq_of_init(struct device_node *np,
		struct device_node *parent)
{
	return owl_sirq_init(s500_sirq_info,
			ARRAY_SIZE(s500_sirq_info), np);
}
IRQCHIP_DECLARE(s500_sirq, "actions,s500-sirq", s500_sirq_of_init);

int __init s900_sirq_of_init(struct device_node *np,
		struct device_node *parent)
{
	return owl_sirq_init(s900_sirq_info,
			ARRAY_SIZE(s900_sirq_info), np);
}
