...
static  struct monitor_data  owl_s500_data = {
	.ic_type = OWL_S500,
...
static const struct of_device_id  owl_monitor_of_match[]   = {
	{.compatible = "actions,s500-usbmonitor", .data = &owl_s500_data },
...
static int monitor_probe(struct platform_device *_dev)
{
	...
	*/
	...
	base = ioremap(USB3_REGISTER_BASE, 0xcd00);
	if (!base) {
	...
	ret = umonitor_core_init(&my_mon->port_ops, &my_mon->port_config, my_mon->base);
	if (ret != 0)
		goto	err_monitor_probe_fail;
