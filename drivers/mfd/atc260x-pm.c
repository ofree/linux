...
static struct atc260x_pm_dev *s_current_atc260x_pm_obj;
...
static const struct atc260x_pm_wakeup_src_reg_desc
    sc_atc2603a_pm_wakeup_src_desc = {
...
static const struct atc260x_pm_wakeup_src_reg_desc
    sc_atc2603c_pm_wakeup_src_desc = {
	.regs = {
	     ATC2603C_PMU_SYS_CTL0, ATC2603C_PMU_SYS_CTL1,
	     ATC2603C_PMU_SYS_CTL3, 0xffff},
	.bit_tbl = {
		    /*  w,   r    */
		    {5, 16 + 5,},	/* OWL_PMIC_WAKEUP_SRC_IR */
		    {6, 16 + 6,},	/* OWL_PMIC_WAKEUP_SRC_RESET */
...
static const struct atc260x_pm_wakeup_src_reg_desc
    sc_atc2609a_pm_wakeup_src_desc = {
...
static const struct atc260x_pm_wakeup_src_reg_desc *const
    sc_atc260x_pm_wakeup_src_desc_tbl[ATC260X_ICTYPE_CNT] = {
					    ...
					    uint wakeup_mask, uint wakeup_src)
{
	...
	wakeup_src_desc =
	    sc_atc260x_pm_wakeup_src_desc_tbl[atc260x_pm->pmic_type];
	...
	while (tmp_mask) {
		...
		if (comb_bit == 255) {
			if (wakeup_src & (1U << src_index)) {
				dev_warn(atc260x_pm->dev,
					 "%s() src %u (mask 0x%x) not support\n",
					 __func__, src_index,
					 (1U << src_index));
			}
			continue;
	...
	for (i = 0; i < ARRAY_SIZE(reg_masks); i++) {
		...
		if (reg_masks[i] != 0 && reg_addr != 0xffff) {
			ret = atc260x_reg_setbits(atc260x_pm->atc260x,
						  reg_addr, reg_masks[i],
						  reg_vals[i]);
			if (ret) {
				dev_err(atc260x_pm->dev,
					"%s() io err, ret=%d\n", __func__, ret);
				return -EIO;
			}
			dev_info(atc260x_pm->dev,
				 "%s() setup reg=0x%x msk=0x%x val=0x%x readback=0x%x\n",
...
static int _atc260x_pm_get_wakeup_src_inner(struct atc260x_pm_dev *atc260x_pm,
					    bool get_flag)
{
	const struct atc260x_pm_wakeup_src_reg_desc *wakeup_src_desc;
...
static int _atc260x_pm_set_wakeup_src(uint wakeup_mask, uint wakeup_src)
{
	...
	return _atc260x_pm_set_wakeup_src_inner(atc260x_pm, wakeup_mask,
						wakeup_src);
}
				       ...
				       *p_old_alarm)
{
	...
	rtc_times = mktime(rtc_cen * 100 + ((rtc_ymd >> 9) & 0x7fU),
			   (rtc_ymd >> 5) & 0xfU,
			   rtc_ymd & 0x1fU,
			   rtc_h, (rtc_ms >> 6) & 0x3fU, rtc_ms & 0x3fU);
	rtc_alm_times = mktime(rtc_cen * 100 + ((rtc_ymdalm >> 9) & 0x7fU),
			       (rtc_ymdalm >> 5) & 0xfU,
			       rtc_ymdalm & 0x1fU,
			       rtc_halm,
			       (rtc_msalm >> 6) & 0x3fU, rtc_msalm & 0x3fU);
	...
	if (rtc_times < rtc_alm_times) {
		...
		p_old_alarm->msalm =
		    ((rtc_tmp_tm.tm_min) << 6) | rtc_tmp_tm.tm_sec;
		p_old_alarm->halm = rtc_tmp_tm.tm_hour;
		p_old_alarm->ymdalm =
		    ((rtc_tmp_tm.tm_year + 1900 - rtc_cen * 100) << 9) |
		    ((rtc_tmp_tm.tm_mon + 1) << 5) | rtc_tmp_tm.tm_mday;
		dev_info(atc260x_pm->dev,
			 "%s() saved old alarm %u-%u-%u %u:%u:%u, "
			 "msalm=0x%x halm=0x%x ymdalm=0x%x\n", __func__,
			 rtc_tmp_tm.tm_year + 1900, rtc_tmp_tm.tm_mon + 1,
			 rtc_tmp_tm.tm_mday, rtc_tmp_tm.tm_hour,
			 rtc_tmp_tm.tm_min, rtc_tmp_tm.tm_sec,
	...
	rtc_time_to_tm(rtc_alm_times, &rtc_tmp_tm);
	rtc_msalm = ((rtc_tmp_tm.tm_min) << 6) | rtc_tmp_tm.tm_sec;
	rtc_halm = rtc_tmp_tm.tm_hour;
	rtc_ymdalm =
	    ((rtc_tmp_tm.tm_year + 1900 - rtc_cen * 100) << 9) |
	    ((rtc_tmp_tm.tm_mon + 1) << 5) | rtc_tmp_tm.tm_mday;
	...
	ret7 =
	    atc260x_reg_write(atc260x_pm->atc260x, reg_rtc_ymdalm, rtc_ymdalm);
	if (ret5 || ret6 || ret7) {
	...
	dev_info(atc260x_pm->dev, "%s() new alarm set to %u-%u-%u %u:%u:%u, "
		 ...
		 __func__,
		 rtc_tmp_tm.tm_year + 1900, rtc_tmp_tm.tm_mon + 1,
		 rtc_tmp_tm.tm_mday, rtc_tmp_tm.tm_hour, rtc_tmp_tm.tm_min,
		 rtc_tmp_tm.tm_sec, rtc_msalm, rtc_halm, rtc_ymdalm);
...
static int _atc260x_pm_enter_suspend(void)
{
	...
	if (!p_cpu_resume || IS_ERR(p_cpu_resume)) {
		dev_err(atc260x_pm->dev, "%s() symbol owl_cpu_resume not found",
			__func__);
		return -EINVAL;
	}
	...
	/* set FW suspend flag */
	ret =
	    atc260x_pstore_set(atc260x_pm->atc260x, ATC260X_PSTORE_TAG_FW_S2,
			       1);
	if (ret) {
		dev_err(atc260x_pm->dev, "%s() %u err\n", __func__, __LINE__);
		return ret;
	...
	for (i = 0; i < 10; i++) {
		ret = atc260x_reg_setbits(atc260x, reg_sysctl3,
					  PMU_SYS_CTL3_EN_S2 |
					  PMU_SYS_CTL3_EN_S3, reg_sysctl3_val);
		if (ret == 0) {
			break;
		}
		dev_err(atc260x_pm->dev, "%s() %u err\n", __func__, __LINE__);
...
static int _atc260x_pm_prepare_powerdown_upgarde(void)
{
	struct atc260x_pm_dev *atc260x_pm;
	struct atc260x_dev *atc260x;
	int ret;
	...
	atc260x_pm = _get_curr_atc260x_pm_obj();
	atc260x = atc260x_pm->atc260x;
	/*reset all pstore reg */
	ret = atc260x_pstore_reset_all(atc260x_pm->atc260x);
	if (ret)
		return ret;
	/*set flag, ensure enter s3 after upgrade */
	ret = atc260x_pstore_set(atc260x_pm->atc260x,
				 ATC260X_PSTORE_TAG_SHDWN_NOT_DEEP, 1);
	if (ret)
		dev_err(atc260x_pm->dev,
			"%s() atc260x_pstore_reset_all err!\n", __func__);
...
static int _atc260x_pm_prepare_powerdown(void)
{
	...
	atc260x_set_reg_direct_access(_get_curr_atc260x_pm_obj()->atc260x,
				      true);
	return 0;
}
...
static int _atc2603a_pm_do_reboot(struct atc260x_pm_dev *atc260x_pm)
{
	...
	if (ret) {
		dev_err(atc260x_pm->dev,
			"%s() clear VBUS&WALL wakeup err, ret=%d\n", __func__,
			ret);
		return ret;
	}
...
static int _atc2603c_9a_pm_do_reboot(struct atc260x_pm_dev *atc260x_pm)
{
	...
	ret = _atc260x_pm_set_wakeup_src_inner(atc260x_pm,
					       ...
					       OWL_PMIC_WAKEUP_SRC_RESET);
	if (ret) {
		dev_err(atc260x_pm->dev,
			"%s() clear setup wakeup err, ret=%d\n", __func__, ret);
		return ret;
	}
...
static int _atc260x_pm_reboot_prepare_upgrade(void)
{
	struct atc260x_pm_dev *atc260x_pm;
	struct atc260x_dev *atc260x;
	int ret;
...
static int _atc260x_pm_reboot(uint tgt)
{
	...
	ret =
	    atc260x_pstore_set(atc260x, ATC260X_PSTORE_TAG_REBOOT_ADFU,
			       flag_adfu);
	ret |=
	    atc260x_pstore_set(atc260x, ATC260X_PSTORE_TAG_REBOOT_RECOVERY,
			       flag_recovery);
	ret |=
	    atc260x_pstore_set(atc260x, ATC260X_PSTORE_TAG_DIS_MCHRG,
			       flag_dis_mchrg);
	if (ret) {
...
static struct owl_pmic_pm_ops atc260x_pm_pmic_ops = {
	...
	.shutdown_prepare_upgrade = _atc260x_pm_prepare_powerdown_upgarde,
	.shutdown_prepare = _atc260x_pm_prepare_powerdown,
	.powerdown = _atc260x_pm_powerdown,
	.reboot_prepare_upgrade = _atc260x_pm_reboot_prepare_upgrade,
					    ...
					    char *buf)
{
	...
	ret = atc260x_reg_read(atc260x_pm->atc260x,
			       sc_atc260x_pm_regtbl_sysctl2[atc260x_pm->
							    pmic_type]);

	if (ret & PMU_SYS_CTL2_S2_TIMER_EN) {
		i = (ret & PMU_SYS_CTL2_S2_TIMER_MASK) >>
		    PMU_SYS_CTL2_S2_TIMER_SHIFT;
					    ...
					    char *buf)
{
	...
	ret = atc260x_reg_read(atc260x_pm->atc260x,
			       sc_atc260x_pm_regtbl_sysctl3[atc260x_pm->
							    pmic_type]);

	if (ret & PMU_SYS_CTL3_S3_TIMER_EN) {
		i = (ret & PMU_SYS_CTL3_S3_TIMER_MASK) >>
		    PMU_SYS_CTL3_S3_TIMER_SHIFT;
		len = sprintf(buf, "[%lu] ", s2tos3_timeout_table[i]);
	} else {
		len = sprintf(buf, "[%d] ", 0);
	}
...
static void _clear_status(struct atc260x_pm_dev *atc260x_pm)
{
	...
	/* disable alarm wake */
	ret =
	    _atc260x_pm_set_wakeup_src_inner(atc260x_pm,
					     OWL_PMIC_WAKEUP_SRC_ALARM, 0);
	if (ret) {
		dev_err(atc260x_pm->dev, "%s() %u err\n", __func__, __LINE__);
	}
	...
	/* restore old alarm */
	ret =
	    atc260x_pstore_get(atc260x, ATC260X_PSTORE_TAG_RTC_MSALM,
			       &rtc_msalm);
	ret |=
	    atc260x_pstore_get(atc260x, ATC260X_PSTORE_TAG_RTC_HALM, &rtc_halm);
	ret |=
	    atc260x_pstore_get(atc260x, ATC260X_PSTORE_TAG_RTC_YMDALM,
			       &rtc_ymdalm);
	if (ret) {
		dev_err(atc260x_pm->dev, "%s() %u err\n", __func__, __LINE__);
	}
	if (rtc_msalm != 0 && rtc_halm != 0 && rtc_ymdalm != 0) {
		ret = atc260x_reg_write(atc260x,
					sc_atc260x_pm_regtbl_rtc_msalm
					[atc260x_pm->pmic_type], rtc_msalm);
		ret |=
		    atc260x_reg_write(atc260x,
				      sc_atc260x_pm_regtbl_rtc_halm[atc260x_pm->
								    pmic_type],
				      rtc_halm);
		ret |=
		    atc260x_reg_write(atc260x,
				      sc_atc260x_pm_regtbl_rtc_ymdalm
				      [atc260x_pm->pmic_type], rtc_ymdalm);
		if (ret) {
	...
	ret |=
	    atc260x_pstore_set(atc260x, ATC260X_PSTORE_TAG_REBOOT_RECOVERY, 0);
...
static int atc260x_pm_setup_wall_vbus_wakup_function(struct atc260x_pm_dev
						     *atc260x_pm)
{
	...
	ret =
	    of_property_read_u32(of_power_node, "support_adaptor_type",
				 &support_adaptor_type);
	if (ret) {
		dev_err(atc260x_pm->dev,
			"%s() can not get of_property support_adaptor_type \n",
			__func__);
		return -ENODEV;
	}
	...
	ret = atc260x_reg_setbits(atc260x_pm->atc260x,
				  sc_atc260x_pm_regtbl_sysctl5[atc260x_pm->
							       pmic_type],
				  reg_mask, reg_val);
	if (ret) {
...
static int atc260x_pm_notifier_callback(struct notifier_block *nb,
...
static int atc260x_pm_probe(struct platform_device *pdev)
{
	...
	if (ret) {
		dev_err(atc260x_pm->dev,
			"%s() setup_wall_vbus_wakup_function failed\n",
			__func__);
	...
	if (ret) {
		dev_err(atc260x_pm->dev,
			"failed to register pm_notifier, ret=%d\n", ret);
		goto label_err_lv2;
	}
	...
	if (ret) {
		dev_err(atc260x_pm->dev,
			"failed to create sysfs nodes for atc260x_pm, ret=%d\n",
			ret);
		goto label_err_lv3;
	}
