...
int read_storage(void *buf, int start, int size)
{
	...
	for (i = 0; i < 6; i++) {
		...
		if (IS_ERR(file) || file->f_op == NULL
			...
			if (IS_ERR(file))
				PRINT("file is err\n");
			else if (file->f_op == NULL)
				PRINT("file->f_op == NULL\n");
			else if (file->f_op->read == NULL)
				PRINT("file->f_op->read == NULL\n");
			if (i < 5) {
				continue;
			} else {
	...
	offset = file->f_op->llseek(file, (mi_base + start), 0);
	if (offset != (mi_base + start)) {
		PRINT_ERR("lseek failed, offset %d\n", (int)offset);
		ret = -1;
		goto OUT;
	}
	count = file->f_op->read(file,
			(unsigned char *)buf, size, &file->f_pos);
...
int read_storage(void *buf, int start, int size)
{
	...
	if (start % BLOCK_SIZE != 0) {
		start_in_blk = (mi_base + start) % BLOCK_SIZE;
		/*from sector 0*/
		start_blk = (mi_base + start) / BLOCK_SIZE;
	...
	while (size > BLOCK_SIZE) {
		start_blk = (mi_base + start) / BLOCK_SIZE;/*from sector 0*/
		if (blk_read(blk_buf, start_blk, 1) < 0) {
			PRINT_ERR("blk_read failed, start blk %d, blkcnt 1\n", start_blk);
			ret = -1;
			goto OUT_FAILED;
...
int write_mi_item(char *name, void *buf, unsigned int count)
{
	...
	packet->item[0].magic = MISC_INFO_MAGIC;
...
int read_misc_info(void *buf, unsigned int count)
{
	...
	for (i = 0; i < head->item_num; i++)
		pack_length += sizeof(packet_item_t) + head->item_head[i].size;
...
int write_misc_info(void *buf, unsigned int count)
{
	...
	new_data = (char *)new_misc + head_length;
	...
	/*!!!!(char *) is must, so easy to forget*/
	old_data = (char *)old_misc + head_length;
	if (old_head->magic != MISC_INFO_MAGIC || old_head->length > MISC_INFO_MAX_SIZE
		...
		for (i = 0; i < old_head->item_num; i++) {
			for (j = 0; j < pack_item_num; j++) {
				if (memcmp(old_head->item_head[i].name, new_head->item_head[j].name,
						sizeof(old_head->item_head[i].name)) == 0)
					break;
			}
			if (j < pack_item_num)
				continue;

			memcpy(&new_head->item_head[index], &old_head->item_head[i],
					sizeof(item_head_t));
			new_head->item_head[index].offset = head_length + offset;
			offset += new_head->item_head[index].size;

			/*copy data old_misc->data is wrong*/
...
static long mi_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case GET_ITEM_DATA:
		...
		break;
	case SET_ITEM_DATA:
		if (copy_from_user(&item, (void *)arg,
				sizeof(ioctl_item_t)) != 0) {
			PRINT_ERR("copy_from_user failed\n");
			goto OUT_NULL;
		}
		PRINT_DBG("item.name %s, len %d\n",
