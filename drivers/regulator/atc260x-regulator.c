...
typedef int (*atc260x_regu_volt_lookup_func_t) (struct atc260x_regulator_dev *
						atc260x_regu,
						const struct atc260x_regu_hwinfo
						*hwinfo, uint band,
						uint index);
...
static int _atc260x_regulator_set_volt_band(struct atc260x_regulator_dev
					    *atc260x_regu, uint sel_band)
{
	...
	ret = atc260x_reg_setbits(atc260x,
				  ...
				  hwinfo->vsel_band_bm,
				  (sel_band << __ffs(hwinfo->vsel_band_bm)) &
				  hwinfo->vsel_band_bm);
	if (ret) {
		dev_err(atc260x_regu->dev,
			"%s() failed, reg=0x%x bm=0x%x ret=%d\n", __func__,
			hwinfo->vsel_band_reg_addr, hwinfo->vsel_band_bm, ret);
	}
	return ret;
}

static int _atc260x_regulator_list_voltage(struct regulator_dev *rdev,
					   unsigned selector)
{
	...
	if (hwinfo->nonlinear_lookup != NULL) {
		ret = (hwinfo->nonlinear_lookup) (atc260x_regu, hwinfo,
						  atc260x_regu->
						  selected_volt_band, selector);
		if (ret < 0) {
			dev_err(atc260x_regu->dev,
				"%s() call nonlinear_lookup err, ret=%d\n",
				__func__, ret);
			return ret;
		}
		result = ret;
		...
		if (hwinfo->volt_band_exclusive) {
			BUG_ON(hwinfo->volt_band_exclusive >=
			       ARRAY_SIZE(hwinfo->volt_bands));
			volt_band =
			    &(hwinfo->
			      volt_bands[atc260x_regu->selected_volt_band]);
		} else {
			volt_band = NULL;
		...
		result = volt_band->volt_min_uv +
		    (selector -
		     volt_band->vsel_start) * volt_band->volt_step_uv;
	}
						    ...
						    unsigned int new_selector)
{
	 ...
	 *所以不能简单地比较selector. */
	old_volt =
	    (atc260x_regu->regulator_desc.ops->list_voltage) (atc260x_regu->
							      regulator,
							      old_selector);
	if (old_volt < 0)
...
static int _atc260x_regulator_get_hw_status(struct atc260x_regulator_dev
					    *atc260x_regu, u8 *p_on,
					    u8 *p_bypass, u8 *p_oc, u8 *p_ov,
					    u8 *p_uv)
{
	const struct atc260x_regu_hwinfo *hwinfo;
	...
	if (p_on || p_bypass) {
		ret =
		    atc260x_reg_read(atc260x_regu->atc260x,
				     hwinfo->ctrl_reg_addr);
		if (ret < 0) {
			dev_err(atc260x_regu->dev,
				"%s() read act260x reg %u err, ret=%u\n",
	...
	for (i = 0; i < ARRAY_SIZE(state_reg_bm_list); i++) {
		...
		if (state_reg_bm_list[i] != 0) {
			...
			*(state_result_list[i]) =
			    ((uint) ret & state_reg_bm_list[i]) ? 1 : 0;
		} else {
...
static int _atc260x_regulator_get_actual_status(struct regulator_dev *rdev)
{
	...
	ret = _atc260x_regulator_get_hw_status(atc260x_regu,
					       &state_on, &state_bypass,
					       &state_oc, &state_ov, &state_uv);
	if (ret) {
...
static int _atc260x_regulator_set_voltage_sel(struct regulator_dev *rdev,
					      unsigned selector)
{
	struct atc260x_regulator_dev *atc260x_regu;
	const struct atc260x_regu_hwinfo *hwinfo;
...
static int _atc260x_regulator_enable_inner(struct regulator_dev *rdev,
					   uint enable)
{
	struct atc260x_regulator_dev *atc260x_regu;
	const struct atc260x_regu_hwinfo *hwinfo;
...
static int _atc260x_regulator_set_bypass(struct regulator_dev *rdev,
					 bool enable)
{
	struct atc260x_regulator_dev *atc260x_regu;
	const struct atc260x_regu_hwinfo *hwinfo;
...
static int _atc260x_regulator_get_bypass(struct regulator_dev *rdev,
					 bool *enable)
{
	struct atc260x_regulator_dev *atc260x_regu;
	const struct atc260x_regu_hwinfo *hwinfo;
...
static int _atc260x_init_regulator_desc(struct atc260x_regulator_dev
					*atc260x_regu,
					struct regulator_desc *r_desc,
					struct regulator_config *r_cfg,
					struct regulator_init_data *init_data)
{
	static const struct atc260x_regu_hwinfo *const
	    sc_atc260x_dcdc_hwinfo_tbl_pool[ATC260X_ICTYPE_CNT] = {
		[ATC260X_ICTYPE_2603A] = sc_atc2603a_dcdc_hwinfo_tbl,
		[ATC260X_ICTYPE_2603C] = sc_atc2603c_dcdc_hwinfo_tbl,
		[ATC260X_ICTYPE_2609A] = sc_atc2609a_dcdc_hwinfo_tbl,
	};
	static const uint sc_atc260x_dcdc_hwinfo_tbl_len[ATC260X_ICTYPE_CNT] = {
		[ATC260X_ICTYPE_2603A] =
		    ARRAY_SIZE(sc_atc2603a_dcdc_hwinfo_tbl),
		[ATC260X_ICTYPE_2603C] =
		    ARRAY_SIZE(sc_atc2603c_dcdc_hwinfo_tbl),
		[ATC260X_ICTYPE_2609A] =
		    ARRAY_SIZE(sc_atc2609a_dcdc_hwinfo_tbl),
	};
	static const struct atc260x_regu_hwinfo *const
	    sc_atc260x_ldo_hwinfo_tbl_pool[ATC260X_ICTYPE_CNT] = {
	...
	if (strstr(atc260x_regu->name, "dcdc") != NULL) {
		...
		dev_err(atc260x_regu->dev,
			"%s() unknown type of regulator dev %s\n", __func__,
			atc260x_regu->name);
		return -ENXIO;
	}
	if (atc260x_regu->regu_id >= hwinfo_tbl_len) {
		dev_err(atc260x_regu->dev,
			"%s() unknown regulator (%s) id %d\n", __func__,
			atc260x_regu->name, atc260x_regu->regu_id);
		return -ENXIO;
	}
	...
	if (hwinfo->volt_bands[1].volt_max_uv == 0) {
		...
		if (hwinfo->volt_band_exclusive == 0) {
			...
			/* 电压配置分了两段, 是互斥的, 只能2选1. */
			if (_atc260x_chk_constraints_vrang
				...
				dev_err(atc260x_regu->dev,
					"%s() unmatched voltage rang [%u,%u]\n",
					__func__, init_data->constraints.min_uV,
					init_data->constraints.max_uV);
				return -ENXIO;
			}
			ret =
			    _atc260x_regulator_set_volt_band(atc260x_regu,
							     selected_volt_band);
			if (ret) {
				return ret;
			}
...
static void _atc260x_dump_regulator_info(struct atc260x_regulator_dev
					 *atc260x_regu,
					 struct regulator_init_data *init_data)
{
#if CONFIG_ATC260x_REGU_FULL_DBG
	...
	dev_info(atc260x_regu->dev,
		 "dump regulator ----------------------------\n");
	pr_info("  id: %u  name: %s\n", atc260x_regu->regu_id,
		atc260x_regu->name);
	...
	for (i = 0; i < ARRAY_SIZE(hwinfo->volt_bands); i++) {
		pr_info
		    ("    volt_bands_%u     : min %uuV  max %uuV  step %uuV  vsel_start %u  vsel_end %u\n",
		     i, hwinfo->volt_bands[i].volt_min_uv,
		     hwinfo->volt_bands[i].volt_max_uv,
		     hwinfo->volt_bands[i].volt_step_uv,
		     hwinfo->volt_bands[i].vsel_start,
		     hwinfo->volt_bands[i].vsel_end);
	}
...
static int atc260x_regulator_probe(struct platform_device *pdev)
{
	...
	ret = _atc260x_init_regulator_desc(atc260x_regu,
					   &(atc260x_regu->regulator_desc),
					   &config, init_data);
	if (ret) {
		dev_err(&pdev->dev,
			"%s() failed to init regulator_desc, ret=%d\n",
			__func__, ret);
		return ret;
	}
	_atc260x_dump_regulator_info(atc260x_regu, init_data);

	atc260x_regu->regulator =
	    regulator_register(&(atc260x_regu->regulator_desc), &config);
	if (IS_ERR(atc260x_regu->regulator)) {
		ret = PTR_ERR(atc260x_regu->regulator);
		dev_err(&pdev->dev,
			"%s() Failed to register regulator, ret=%d\n", __func__,
			ret);
		return ret;
	}
	...
	platform_device_register_resndata(NULL, "reg-virt-consumer",
					  (strstr(atc260x_regu->name, "dcdc") ?
					   0 : 10) + atc260x_regu->regu_id,
					  NULL, 0, atc260x_regu->name,
					  strlen(atc260x_regu->name) + 1);
#endif
	return 0;
}
...
static int atc260x_regulator_print_suspend_state(struct atc260x_regulator_dev
						 *atc260x_regu,
						 const char *prompt)
{
	u8 state_on, state_bypass;
	...
	if (((state_on == 1)
	     && (atc260x_regu->regulator->constraints->always_on == 0))
	    || (state_bypass | state_oc | state_ov | state_uv)) {
		/* 有异常状态时提高打印等级. */
		dev_warn(atc260x_regu->dev,
			 "state %s : on=%u bypass=%u oc=%u ov=%u uv=%u\n",
			 prompt, state_on, state_bypass, state_oc, state_ov,
			 state_uv);
	} else {
		dev_info(atc260x_regu->dev,
			 "state %s : on=%u bypass=%u oc=%u ov=%u uv=%u\n",
...
static int atc260x_regulator_suspend_late(struct device *dev)
{
	...
	if (atc260x_regu->regulator->constraints->state_mem.disabled) {
		if (_atc260x_regulator_is_enabled(atc260x_regu->regulator)) {
			...
			ret =
			    _atc260x_regulator_disable(atc260x_regu->regulator);
			if (ret) {
	...
	/* 打印状态, 方便休眠漏电的调试. */
	return atc260x_regulator_print_suspend_state(atc260x_regu,
						     "@suspend_late");
}
...
static int atc260x_regulator_resume_early(struct device *dev)
{
	...
	ret =
	    _atc260x_regulator_set_voltage_sel(atc260x_regu->regulator,
					       atc260x_regu->
					       suspend_save_selector);
	if (ret) {
	...
	if (atc260x_regu->regulator->constraints->state_mem.disabled) {
		if (atc260x_regu->suspend_save_state != 0) {
			ret =
			    _atc260x_regulator_enable(atc260x_regu->regulator);
			if (ret) {
...
{
	atc260x_regulator_print_suspend_state(platform_get_drvdata(pdev),
					      "before shutdown");
}
...
static int __init atc260x_regulator_init(void)
{
	...
	if (ret != 0) {
		pr_err
		    ("%s(): failed to register ATC260X regulator driver, ret=%d\n",
		     __func__, ret);
	}
	return ret;
}
