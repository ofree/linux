...
static int atc260x_switch_ldo_is_enabled(struct regulator_dev *rdev)
{
	...
	if (1 == id) {
		mask_bit =
		    PMU_SWITCH_CTL_SWITCH1_EN | PMU_SWITCH_CTL_SWITCH1_MODE_LDO;
		ret &= mask_bit;
		...
		mask_bit =
		    PMU_SWITCH_CTL_SWITCH2_EN | PMU_SWITCH_CTL_SWITCH2_MODE_LDO;
		ret &= mask_bit;
		return (ret == mask_bit);
	}
}
...
static int ldo_init(struct atc260x_swldo_dev *ldo,
		    struct regulator_init_data *init_data)
{
	...
	if (id == 1) {
		...
		if ((ldo->pmic_type == ATC260X_ICTYPE_2603A) &&
			...
			pr_warn
			    ("[SWITCH1] set switch 1 to ldo mode (2603a Version A~C!)\n");
			atc260x_reg_setbits(ldo->atc260x, ldo->ctrl_reg,
			...
			pr_warn
			    ("[SWITCH1] set switch 1 to ldo mode Version D!\n");
			atc260x_reg_setbits(ldo->atc260x, ldo->ctrl_reg,
					    PMU_SWITCH_CTL_SWITCH1_MODE_MASK,
					    0);
		}
		...
		if ((switch2_ldo_volcfg1_table[0] <= min_uv) &&
			...
			switch2_SEL = PMU_SWITCH_CTL_SWITCH2_LDO_VOL_SEL_10_20V;
			   ...
			   (switch2_ldo_volcfg2_table[table2_len - 1] >=
			    max_uv)) {
			atc260x_reg_setbits(ldo->atc260x, ldo->ctrl_reg,
					    PMU_SWITCH_CTL_SWITCH2_LDO_VOL_SEL_MASK,
...
static int atc260x_switch_ldo_resume_early(struct device *dev)
{
	...
	if (1 == id) {
		ret = atc260x_reg_setbits(ldo->atc260x, ldo->ctrl_reg,
					  PMU_SWITCH_CTL_SWITCH1_LDO_MASK,
					  PMU_SWITCH_CTL_SWITCH1_LDO_SET(ldo->
									 vsel_now));
	} else {
		ret = atc260x_reg_setbits(ldo->atc260x, ldo->ctrl_reg,
					  PMU_SWITCH_CTL_SWITCH2_LDO_MASK |
					  PMU_SWITCH_CTL_SWITCH2_LDO_VOL_SEL_MASK,
					  PMU_SWITCH_CTL_SWITCH2_LDO_SET(ldo->
									 vsel_now))
		    | switch2_SEL;
	}
