...
static inline void owl_dma_dump_lli(struct owl_dma_vchan *vchan,
				    struct owl_dma_lli *lli)
{
	...
	dev_dbg(chan2dev(&vchan->vc.chan),
		...
		p_lli, &lli->hw,
		lli->hw.next_lli, lli->hw.saddr, lli->hw.daddr,
		lli->hw.flen, lli->hw.fcnt, lli->hw.ctrla,
		lli->hw.ctrlb, lli->hw.const_num);
}
			...
			struct dma_slave_config *sconfig, bool is_cyclic)
{
	...
	if (is_cyclic)
		hw->ctrlb = llc_hw_ctrlb(DMA_INTCTL_BLOCK);
	else
		hw->ctrlb = llc_hw_ctrlb(DMA_INTCTL_SUPER_BLOCK);

	owl_dma_dump_lli(vchan, lli);
...
#if 1
	/* for debug only */
	if (pchan_readl(pchan, DMAX_START)) {
		u32 src = pchan_readl(pchan, DMAX_SOURCE);
		u32 dest = pchan_readl(pchan, DMAX_DESTINATION);

		/* skip checking for i2s/serial3(bluetooth) dma */
		if ((src != 0xe010000c) &&
		    (dest != 0xe0100008) &&
		    (dest != 0xe0100020) &&
		    (src != 0xe0126004) &&
		    (dest != 0xe0126008)) {
			/* stop dma to reproduct timeout */
			pchan_writel(pchan, 0, DMAX_START);

			dev_warn(od->dma.dev,
				"%s: warning: terminate pchan%d that still "
				"busy(rlen %x, intstat %x)\n",
				__func__,
				pchan->id,
				pchan_readl(pchan, DMAX_REMAIN_CNT),
				pchan_readl(pchan, DMAX_INT_STATUS));
...
static int owl_dma_probe(struct platform_device *pdev)
{
	...
	pr_info("[OWL] initialize DMA controller\n");
